...
class SelectiveRepeatUI(ProtocolPlugin):
    ...
    def tick(self, k: int) -> int:
        ...
        try:
            for _ in range(k):
                run_sr_bidirectional(steps=1, max_seq=self.runner.cfg.max_seq)
            return k
    ...
    def auto_step(self) -> int:
        ...
        try:
            ...
            run_sr_bidirectional(steps=BLOCK, max_seq=self.runner.cfg.max_seq)
            return BLOCK
        except Exception as e:
            messagebox.showerror(self.name, str(e))
            return 0
    ...
    def direction_for(self, kind: str, seq: int, ack, info: str) -> str:
        ...
        if _norm_kind(kind) == "DATA":
            ...
            if s.startswith("ACK:A"): return "LR"  # A→B
            if s.startswith("ACK:B"): return "RL"  # B→A
            return "RL"